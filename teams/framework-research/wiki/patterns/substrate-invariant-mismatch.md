...
amendments:
  ...
  - date: 2026-05-14
    change: Watch note added — `references/members-array-edit-honored-mid-session.md` (companion-pair sibling of wake-mechanism reference) carries an implicit substrate invariant ("plain JSON file edits to `members[]` are honored on the next `SendMessage` validation"). n=2 cross-substrate verification (FR Windows-Git-Bash + apex Linux/Docker, 2026-05-14) shows the invariant holds across two substrates currently. If a future substrate (Anthropic harness change, alternative LLM provider, sandboxed deployment) does NOT honor on-demand file edits — e.g., by introducing a member-list cache with non-trivial TTL or by moving `members[]` to an authoritative in-memory store — the dependent artifacts (ghost-bridge daemon, on-demand library-team provisioning per service-team-topology) become instances of this defect class. No new instance added (n=6 unchanged) — this is a forward-watch note, not a current defect. Sources: callimachus (filer).
---
